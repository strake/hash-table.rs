...
pub struct HashTable<K: Eq + Hash, T,
                     ...
                     Es: IndexMut<usize, Output = (K, T)>, H: Clone + Hasher = DefaultHasher> {
    hashes: Hs,
    elems : Es,
    free_n: usize,
    hasher: H,
...
impl<K: Eq + Hash, T, Hs: IndexMut<usize, Output = usize> + Index<RangeFull, Output = [usize]>,
     Es: IndexMut<usize, Output = (K, T)>, H: Clone + Hasher> HashTable<K, T, Hs, Es, H> {
    ...
    fn find_ix<Q: ?Sized>(&self, k: &Q) -> Option<usize> where K: Borrow<Q>, Q: Eq + Hash {
        ...
        loop {
            ...
            if self.hashes[i] == h && self.elems[i].0.borrow() == k { return Some(i); }
            i = (i+1)&wrap_mask;
    ...
    pub fn find_with_ix<Q: ?Sized>(&self, k: &Q) -> Option<(usize, &K, &T)> where K: Borrow<Q>, Q: Eq + Hash {
        self.find_ix(k).map(move |i| (i, &self.elems[i].0, &self.elems[i].1))
    }

    #[inline]
    pub fn find_mut_with_ix<Q: ?Sized>(&mut self, k: &Q) -> Option<(usize, &K, &mut T)> where K: Borrow<Q>, Q: Eq + Hash {
        self.find_ix(k).map(move |i| { let &mut (ref k, ref mut v) = &mut self.elems[i]; (i, k, v) })
    }
    ...
    pub fn insert_with<F: FnOnce(Option<T>) -> T>(&mut self, k: K, f: F) -> Result<(usize, &K, &mut T), (K, F)> {
        ...
        loop {
            if self.hashes[i] == 0 {
                self.hashes[i] = h;
                unsafe { ptr::write(&mut self.elems[i], (k, f(None))); }
                let &mut (ref k, ref mut v) = &mut self.elems[i];
                return Ok((i, k, v))
            }

            if self.hashes[i] == h && self.elems[i].0 == k {
                unsafe {
                    self.hashes[i] |=  dead_flag;
                    let x = ptr::read(&self.elems[i].1);
                    ptr::write(&mut self.elems[i].1, f(Some(x)));
                    self.hashes[i] &= !dead_flag;
                }
                let &mut (ref k, ref mut v) = &mut self.elems[i];
                return Ok((i, k, v))
            ...
            if psl > compute_psl(&self.hashes[..], i) {
                ...
                loop {
                    ...
                    mem::swap(&mut e, &mut self.elems[i]);
                    if h == 0 || is_dead(h) {
                        mem::forget(e);
                        let &mut (ref k, ref mut v) = &mut self.elems[i];
                        return Ok((i, k, v));
